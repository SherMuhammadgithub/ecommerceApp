  ...
  return (
    ...
    <section className="text-gray-600 body-font">
      <div className="container px-5 py-24 mx-auto">
        ...
        {Object.keys(products).length === 0 && <p>Sorry all the mugs are currently out of stock! New Stock is comming soon Sray tuned!</p> }
...
export async function getServerSideProps() {
  ...
  for (let item of products) {
    if (item.title in mugs) {
      if (!mugs[item.title].color.includes(item.color) && item.availbleQty > 0) {
        mugs[item.title].color.push(item.color)
      }
      if (!mugs[item.title].size.includes(item.size) && item.availbleQty > 0) {
