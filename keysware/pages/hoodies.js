...
const Hoodies = ({ products }) => {
  ...
  //ye hoods ha sare products output daikho
  //because i have inserted no product of hood in my database
  //lets insert a *
  return (
    ...
    <section className="text-gray-600 body-font">
      <div className="container px-5 py-24 mx-auto">
        ...
        {Object.keys(products).length === 0 && <p>Sorry all the hoodies are currently out of stock! New Stock is comming soon Sray tuned!</p> }
...
export async function getServerSideProps() {
  ...
  for (let item of products) {
    if (item.title in Hoods) {
      if (!Hoods[item.title].color.includes(item.color) && item.availbleQty > 0) {
        Hoods[item.title].color.push(item.color)
      }
      if (!Hoods[item.title].size.includes(item.size) && item.availbleQty > 0) {
